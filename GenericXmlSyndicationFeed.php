...
class GenericXmlSyndicationFeed {
  ...
  public function __set($k, $v) {
    if (array_search($k, $this->allowedVars) !== false) {
      $this->_vars[$k] = $v;
    }
    else {
  ...
  protected static function MyXMLEncode($s) {
    	$string = preg_replace( '/[\x00-\x08\x0b\x0c\x0e-\x1f]/', '', $s );
  ...
  protected function getRSS20($returnDOM = false) {
    ...
    foreach ($this->items as $i) {
      ...
      $iDesc = $doc->createElement('description', self::MyXMLEncode($i->content));
...
class GenericXmlSyndicationFeedItem {
  ...
  public function appendValue($k, $v) {
    if (array_search($k, $this->allowedVars) !== false && array_search($k, $this->allowedArrays) !== false) {
      $this->_vars[$k][] = $v;
    }
    else {
